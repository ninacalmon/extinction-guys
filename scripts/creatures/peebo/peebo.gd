...
func move_to(next: Vector2i):
	...
	tween.tween_property(self, "global_position", target, TimeManager.current_turn_speed)
	await tween.finished
	sprite.play("idle")


func can_reproduce() -> bool:
	return age >= MAX_AGE / 10.0 and child_count < MAX_CHILDREN
...
func reproduce(partner: Peebo):
	...
	var dir = Globals.get_valid_directions(tile_pos).pick_random()

	Globals.creature_instanciator.create_new_egg(tile_pos + dir)
